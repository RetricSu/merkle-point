 ...
 * @returns {string}
 */
export function hash_data(d) {
  let deferred1_0;
  let deferred1_1;
  try {
    const ret = wasm.hash_data(d);
    deferred1_0 = ret[0];
    deferred1_1 = ret[1];
 ...
 * @returns {boolean}
 */
export function verify_proof(root, proof, leaves) {
  ...
  const ret = wasm.verify_proof(ptr0, len0, ptr1, len1, leaves);
  return ret !== 0;
}
...
export class CkbSmt {
  ...
  update(key, val) {
    ...
    const len1 = WASM_VECTOR_LEN;
    wasm.ckbsmt_update(this.__wbg_ptr, ptr0, len0, ptr1, len1);
  }
  /**
   * @param {string[]} keys
   * @returns {string}
...
async function __wbg_init(module_or_path) {
  ...
  if (typeof module_or_path === "undefined") {
    module_or_path = new URL("smt_wasm_bg.wasm", import.meta.url);
  }
  const imports = __wbg_get_imports();
